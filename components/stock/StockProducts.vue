<template>
  <v-card class="product-container">
    ...
    <video muted playsinline width="200" height="200" id="qrScanner"></video>
...
<script>
  ...
  export default {
    ...
    watch: {
      ...
      products (val) {
        this.productsData = _.cloneDeep(val);
      }
    },
    created () {
      this.productsData = _.cloneDeep(this.products);
    },
    mounted () {
      QrScanner.WORKER_PATH = '/scripts/qr-scanner-worker.min.js';
      const qrScanner = new QrScanner(document.getElementById('qrScanner'), result => console.log('decoded qr code:', result));
      qrScanner.start();
    },
    methods: {
      addRequest (product) {
        ProductApi.add(product)
          .then((data) => this.$emit('getProducts'))
      ...
      deleteRequest (id) {
        ProductApi.delete({ id: id })
          .then((data) => this.$emit('getProducts'))
          .catch(error => console.log(error));
      },
      editItem (item) {
        this.editedIndex = this.productsData.indexOf(item);
        this.editedItem = Object.assign({}, item);
        this.dialog = true;
      },
      deleteItem (item) {
        confirm('Вы уверены, что хотите удалить этот товар?') && this.deleteRequest(item._id);
      },
      close () {
        this.dialog = false;
        setTimeout(() => {
